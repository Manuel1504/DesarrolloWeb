<div class="col-md-12 text-center">
  ...
  <div class="col-md-8">
    <form action="<?php echo APP_URL."/Cuentas/edit/".$Datos["id"];;?>" method="post">
      <?PHP  ?>
      ...
      <p>
        <label for="type_id">Usuario</label>
        <select class="form-control" name="user_id">
          <?php  foreach ($Usuario as $user) {?>
            <?php  if ($user["users"]["id"]==$Datos["user_id"]) {?>
              <option selected value="<?php echo $user["users"]["id"];  ?>"> <?php echo $user["users"]["username"];?></option>
            <?php }else{?>
